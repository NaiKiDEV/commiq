...
export function TimelineChart({ timeline, storeNames }: TimelineChartProps) {
  ...
  const selectedChain = useMemo(() => {
    ...
    let cur: TimelineEntry | undefined = selectedEvent;
    while (cur) {
      ids.add(cur.correlationId);
      cur = cur.causedBy ? entryMap.get(cur.causedBy) : undefined;
      if (cur && ids.has(cur.correlationId)) break;
    }

    const queue = [selectedEvent.correlationId];
    while (queue.length > 0) {
      const id = queue.shift()!;
      for (const e of timeline) {
        if (e.causedBy === id && !ids.has(e.correlationId)) {
  ...
  const layout = useMemo(() => {
    ...
    for (const p of positioned) {
      if (p.entry.causedBy && posMap.has(p.entry.causedBy)) {
        links.push({ from: posMap.get(p.entry.causedBy)!, to: p });
      }
    }
          ...
          {!layout ? (
            ...
            >
              ...
              {layout.links.map((lk, i) => {
                ...
                const opacity = selectedChain ? (inChain ? 0.65 : 0.08) : 0.3;
                const stroke = inChain ? colors.accentLight : colors.accent;
                const sw = inChain ? 1.8 : 1.2;
                ...
                if (crossLane) {
                  ...
                  return (
                    <path
                      key={`lk-${i}`}
                      d={`M${lk.from.x},${lk.from.y} C${lk.from.x + dx * 0.35},${lk.from.y} ${lk.to.x - dx * 0.35},${lk.to.y} ${lk.to.x},${lk.to.y}`}
                      fill="none"
                      stroke={stroke}
                      strokeWidth={sw}
                      opacity={opacity}
