...
Each `useSelector` call subscribes independently. A component that only reads `status` will not re-render when `name` changes. For encapsulating this into a reusable hook, see [domain hooks](/patterns/domain-hooks).
