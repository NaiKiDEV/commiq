...
description: What changed between commiq 1.x and 2.0, why it changed, and the edits you need to make.
---
...
All eight packages move to `2.0.0` together and are only tested against each other. Upgrade them in one step.

<Tabs groupId="pkg-manager" items={['pnpm', 'npm', 'Yarn', 'Bun']}>
  <Tab value="pnpm">
    ```bash
    pnpm up '@naikidev/commiq*@^2.0.0'
    ```
  </Tab>
  <Tab value="npm">
    ```bash
...
1. Bump all `@naikidev/commiq*` packages to `^2.0.0` and re-install so peer dependencies resolve to one core copy.
